...
is to drag+drop Intuitives to /Applications, then open /Applications in Finder,
...
Intuitives DAW, however, there is a workaround.  You can open a Terminal and
run Intuitives DAW from the terminal, and grant the terminal access to the
microphone when prompted.  This is the command to run Intuitives DAW from the
terminal after installing:
```
/Applications/Intuitives\ DAW.app/Contents/MacOS/intuitives
```

## Crashes on older, unsupported Intel Macs
We do our very best to support Intuitives DAW on the earliest Intel Macs.
...
Intuitives DAW is fast enough and capable of running on 2009-era hardware.
...
If you would like to install Intuitives DAW for MacOS to a flash drive, or you
...
`_intuitives_home` next to it.  Note that the MacOS app bundle can be installed
