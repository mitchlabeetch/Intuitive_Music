...
class abstract_custom_oscillator:
    """
    PURPOSE: Base interface for user-defined wavetable oscillators.
    ACTION: Provides the foundational structure (widget and layout) for custom synth OSC editors.
    MECHANISM: 
        1. Initializes a QWidget container with a QVBoxLayout.
        2. Defines the abstract open_settings method for state synchronization.
    """
    def __init__(self):
        self.widget = QWidget()
...
def global_get_sine(a_size: int, a_phase: float) -> numpy.ndarray:
...
class additive_osc_amp_bar(QGraphicsRectItem):
    """
    PURPOSE: A single vertical bar representing the amplitude or phase of a harmonic.
    ACTION: Adjusts height and color based on the selected value (dB or phase offset).
    MECHANISM: 
        1. Encapsulates a QGraphicsRectItem.
        2. In set_value, updates the vertical position and height to extend to the scene bottom.
        3. Uses ADD_OSC_FILL (rainbow gradient) to indicate frequency intensity.
    """
    def __init__(self, a_x_pos):
        """Initializes the bar at a fixed horizontal position."""
...
class additive_wav_viewer(QGraphicsView):
    """
    PURPOSE: Displays the final composite waveform generated by the additive harmonics.
    ACTION: Renders a synchronized time-domain graph of one cycle of the wavetable.
    MECHANISM: 
        1. Inherits from QGraphicsView and hosts a QGraphicsScene.
        2. In draw_array, converts a numpy array of amplitude samples into a QPainterPath of connected line segments.
        3. Uses a rainbow gradient fill for the resulting waveform area.
    """
    def __init__(self):
        """Initializes the waveform viewer with fixed scene dimensions."""
...
class additive_osc_viewer(QGraphicsView):
    """
    PURPOSE: The interactive editor for individual harmonics.
    ACTION: Handles mouse gestures to "draw" harmonic amplitudes or phases across the spectrum.
    MECHANISM: 
        1. Manages a collection of additive_osc_amp_bar objects.
        2. Overrides mouse events to calculate which harmonic bar is under the cursor.
        3. Triggers wavetable regeneration on ogni movement for real-time sonic feedback.
    """
    def __init__(
        self,
        a_draw_callback,
        ...
        """Initializes the harmonic editor and instantiates the control bars."""
        QGraphicsView.__init__(self)
...
class custom_additive_oscillator(abstract_custom_oscillator):
    """
    PURPOSE: A high-level synth engine UI for complex additive synthesis.
    ACTION: Orchestrates multiple harmonic editors (amplitude/phase) and a master waveform preview.
    MECHANISM: 
        1. Provides 3 independent additive slots via a QComboBox.
        2. Includes standard synthesis "Tools" (Saw, Square, Triangle, Sine) that mathematically generate harmonic profiles.
        3. Uses numpy to perform additive summation of sines based on the user's harmonic drawing.
        4. Transmits resulting 1024-sample wavetables to the audio engine via a pipe-delimited string.
    """
    def __init__(self, a_configure_callback=None, a_osc_count=3):
        """Builds the additive synth interface with dual editors and tools."""
