...
Based on technology from Stargate DAW
...
int main(int argc, char **argv) {
  ...
  float f_converted_fast_hz =
      f_pit_midi_note_to_hz_fast(f_note_pitch, f_pitch_core);
  ...
  f_i = 0;
  // Reset the phase by running it directly through the filter, so as not to
  // disrupt the filter's state
  ...
  while (f_i < 1000000) {
    v_svf_run_4_pole_lp(
        f_svf_lp, v_svf_run_2_pole_hp(f_svf, f_osc_run_unison_osc(f_osc)));
    f_i++;
  }
  ...
  if (f_osc->osc_cores[0]->output >= 0.5f) {
    while (f_osc->osc_cores[0]->output > 0.5f) {
      v_pkq_run(f_eq1, v_svf_run_2_pole_hp(f_svf, f_osc_run_unison_osc(f_osc)),
                0.0f);
  ...
  while (f_i < WT_FRAMES_PER_CYCLE) {
    float f_current_sample =
        f_osc_run_unison_osc(f_osc) + (f_run_pink_noise(f_noise) * 0.25f);
    v_pkq_run(f_eq1, v_svf_run_2_pole_hp(f_svf, f_current_sample), 0.0f);
  ...
  if (f_osc->osc_cores[0]->output >= 0.5f) {
    while (f_osc->osc_cores[0]->output > 0.5f) {
      v_pkq_run(f_eq1, v_svf_run_2_pole_hp(f_svf, f_osc_run_unison_osc(f_osc)),
                0.0f);
      v_pkq_run(f_eq2, f_eq1->output0, 0.0f);
  ...
  if (f_osc->osc_cores[0]->output >= 0.5f) {
    while (f_osc->osc_cores[0]->output > 0.5f) {
      v_pkq_run(
          f_eq1,
          v_svf_run_2_pole_hp(f_svf, f_osc_run_unison_osc(f_osc) +
                                         (f_run_white_noise(f_noise) * 0.25f)),
          0.0f);
      v_pkq_run(f_eq2, f_eq1->output0, 0.0f);
      v_svf_run_4_pole_lp(f_svf_lp, f_eq2->output0);
    }
  ...
  while (f_i < WT_FRAMES_PER_CYCLE) {
    float f_current_sample =
        f_osc_run_unison_osc(f_osc) + (f_run_white_noise(f_noise) * 0.25f);
    v_pkq_run(f_eq1, v_svf_run_2_pole_hp(f_svf, f_current_sample), 0.0f);
    v_pkq_run(f_eq2, f_eq1->output0, 0.0f);
  ...
  while (f_i < WT_FRAMES_PER_CYCLE) {
    tmp[f_i] = v_svf_run_4_pole_lp(
        f_svf_lp, v_svf_run_2_pole_hp(f_svf, f_run_pink_noise(f_noise)));
    f_i++;
  }
  ...
  while (f_i < WT_FRAMES_PER_CYCLE) {
    tmp[f_i] = v_svf_run_2_pole_lp(
        f_svf_lp, v_svf_run_2_pole_hp(f_svf, f_run_white_noise(f_noise)));
    f_i++;
  }
  ...
  while (f_i < WT_FRAMES_PER_CYCLE) {
    ...
    tmp[f_i] = v_svf_run_4_pole_lp(
        f_svf_lp,
        f_clp_clip(f_clip, v_svf_run_2_pole_lp(f_svf, f_current_sample)));
  ...
  if (f_osc->osc_cores[0]->output >= 0.5f) {
    while (f_osc->osc_cores[0]->output > 0.5f) {
      float f_current_sample =
          f_osc_run_unison_osc(f_osc) + (f_run_pink_noise(f_noise) * 0.25f);
      tmp[f_i] = v_svf_run_4_pole_lp(
          f_svf_lp, v_svf_run_2_pole_hp(f_svf, f_current_sample));
    }
  ...
  if (f_osc->osc_cores[0]->output >= 0.5f) {
    while (f_osc->osc_cores[0]->output > 0.5f) {
      float f_current_sample =
          v_svf_run_2_pole_hp(f_svf, f_osc_run_unison_osc(f_osc) +
                                         (f_run_pink_noise(f_noise) * 0.25f));
      tmp[f_i] = v_svf_run_4_pole_lp(f_svf_lp, f_current_sample);
    }
