...
# Brand - Intuitives DAW
MAJOR_VERSION = 'intuitives'
...
# Check if the exe was run from a flash drive, with a '_intuitives_home' file
# created in the same directory
if IS_WINDOWS:
    dirname = os.path.dirname(sys.executable)
    if os.path.isfile(
        os.path.join(dirname, '..', '_intuitives_home'),
    ):
        USER_HOME = os.path.abspath(
            os.path.join(dirname, '..'),
        )
        print(
            f"Using {USER_HOME} for USER_HOME because _intuitives_home "
            "file exists"
        )
