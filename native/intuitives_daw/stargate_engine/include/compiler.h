...
#define INTUITIVES_VERSION "intuitives"
// Legacy alias for compatibility
#define STARGATE_VERSION INTUITIVES_VERSION
 ...
 */
void sg_assert(int cond, char *msg, ...) __attribute__((format(printf, 2, 3)));
...
size_t sg_snprintf(char *str, size_t size, char *fmt, ...)
    __attribute__((format(printf, 3, 4)));

// Sprintf function specifically for joining paths together
size_t sg_path_snprintf(SGPATHSTR *str, size_t size, SGPATHSTR *fmt,
                        ...); // __attribute__((format(printf, 3, 4)));
