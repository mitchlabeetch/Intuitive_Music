/**
 * INTUITIVES DAW - Core Engine Header
 *
 * This header provides the core engine structures and functions.
 * Legacy code using STARGATE_H will continue to work.
 */
...
typedef struct {
  void (*run)(int sample_count, struct SamplePair *output,
...
double v_print_benchmark(char *a_message, struct timespec a_start,
...
void v_sg_set_playback_pos(t_sg_seq_event_list *self, double a_beat,
                           long a_current_sample);
void g_pitchbend_init(t_seq_event *f_result, SGFLT a_start, SGFLT a_value,
                      int channel);
void g_cc_init(t_seq_event *f_result, int a_cc_num, SGFLT a_cc_val,
               SGFLT a_start, int channel);
void g_note_init(t_seq_event *f_result, int a_note, int a_vel, SGFLT a_start,
                 SGFLT a_length, SGFLT pan, SGFLT attack, SGFLT decay,
                 SGFLT sustain, SGFLT release, int channel, SGFLT pitch_fine);
void v_set_control_from_atm(t_seq_event *event, int a_plugin_uid,
                            t_track *f_track);
void v_open_track(t_track *a_track, SGPATHSTR *a_tracks_folder, int a_index);
void v_buffer_mix(int a_count, struct SamplePair *a_buffer_src,
                  struct SamplePair *a_buffer_dest);
void g_sg_seq_event_list_init(t_sg_seq_event_list *self);
void v_sample_period_split(t_sample_period_split *self,
                           struct SamplePair *a_buffers,
                           struct SamplePair *a_sc_buffers, int a_sample_count,
                           double a_period_start_beat, double a_period_end_beat,
                           double a_event1_beat, double a_event2_beat,
                           long a_current_sample, SGFLT *a_input_buffer,
                           int a_input_count);
...
void v_sample_period_set_atm_events(t_sample_period *self,
                                    ...
                                    long a_current_sample, int a_sample_count);
void v_sg_seq_event_result_set_default(t_sg_seq_event_result *self,
                                       t_sg_seq_event_list *a_list,
                                       struct SamplePair *a_buffers,
                                       SGFLT *a_input_buffers,
                                       int a_input_count, int a_sample_count,
                                       long a_current_sample);
void v_sg_seq_event_list_set(t_sg_seq_event_list *self,
                             t_sg_seq_event_result *a_result,
                             struct SamplePair *a_buffers,
                             SGFLT *a_input_buffers, int a_input_count,
                             int a_sample_count, long a_current_sample,
                             int a_loop_mode);
void g_seq_event_result_init(t_sg_seq_event_result *self);
int i_beat_count_to_samples(double a_beat_count, SGFLT a_tempo, SGFLT a_sr);
double f_samples_to_beat_count(int a_sample_count, double a_tempo,
                               SGFLT sr_recip);
double f_samples_to_beat_count_sr(int a_sample_count, double a_tempo, SGFLT sr);
double f_bpm_to_seconds_per_beat(double a_tempo);
void v_set_control_from_cc(t_seq_event *event, t_track *f_track);
void v_sg_configure(const char *a_key, const char *a_value);
void v_run_main_loop(int sample_count, struct SamplePair *a_buffers,
                     PluginData *a_input_buffers);
void v_run(struct SamplePair *buffers, SGFLT *a_input, int sample_count);
void stop_preview();
